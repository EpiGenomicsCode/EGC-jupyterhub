...
**Note: CILogon integration requires a registered hostname**
 ...
 - Make sure your netID is registered as an admin user in the JupterHub before you add CILogon authentication so that you can login after you turn it on!

**Note: Installing R into the JupyerHub requires distinct installation instructions detailed below**

While the 01-03 deployment scripts be run sequentially from the command line inside the server, for debugging reasons, it is recommended to manually copy and paste the commands and confirm the changes to the system at regular checkpoints which are detailed below.

## Deploying JupyterHub (aka 'what to check at each stage')

1. This provides a local sudo account and ssh password authentication. You can check that this step worked by logging in through Terminal/PuTTY. This step is optional as you can operate through the default Ubuntu account, but can be useful if multiple users have access to this account.

2. JupyterHub should now be deployed on your system although it should be hosted through the IP address and is not SSL-secured.
...
HTTPS through LetsEncrypt **requires** a registered hostname, both of which are required for CILogon authentication. You can run the rest of the deployment scripts without HTTPS and CILogon authentication, but your mileage may vary.
...
5. To install R-compatibility within the JupyerHub, you must login to the JupyerHub and run the provided commands within the Terminal-emulator inside the Hub as an **admin**
...
6. Similar to installing R as sudo within the JupyerHub terminal (see Step 5), the commands within 06_install_dependency.txt are meant to be run within the JupyerHub terminal. This is because the python environment within the JupyerHub is sandboxed from the Ubuntu server. Installing packaged on the server will NOT result in that code being available to users in the JupyterHub and may break the entire system if competing dependencies are enabled.
